...
class FixedEmailBuildersTests(TestCase):
    def test_no_contact_builder(self):
        ...
        body = build_no_contact_body('Sam', 'Priya Patel', 'mentor')
        self.assertIn('Hi Sam,', body)
        self.assertIn('have not been in touch recently', body)
        self.assertIn('Priya Patel', body)
        self.assertIn('mentor', body)
        self.assertIn(f'{settings.FRONTEND_URL}/messages', body)
